...
const ExpertiesPageModel = (prop: incomingData) => {
  ...
  const fiiterout = expertise?.filter((index) =>
    index.toLowerCase().includes(searchQuery?.name?.toLowerCase() || 0)
  );
  ...
  return (
    ...
    >
      <div
        className={`w-[70%] laptop:w-[50%] h-[60vh] rounded-lg p-4 relative ${
        ...
        <div className="flex flex-col justify-center items-center ">
          ...
          <div className="flex justify-center items-center h-[10vh] w-full">
            <input
              ...
              className={`rounded-full w-full h-[6vh] border-2  placeholder:font-[500]${
                isDark ? "text-black" : ""
              }`}
            />
          </div>
              ...
              <div className="grid grid-cols-3 gap-4 p-2">
                {expertise?.map((item, index) => (
                  <div key={index} className="p-2">
                    <button
                      type="button"
                      className={`w-full h-[7vh] rounded-full border-2 flex justify-evenly items-center ${
